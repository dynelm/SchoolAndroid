...
public class KqwSpeechCompound {
    ...
    private CallBackTTS callBackTTS;
    private static volatile KqwSpeechCompound kqwSpeechCompound;
    ...
    private SynthesizerListener mTtsListener = new SynthesizerListener() {
        ...
        public void onCompleted(SpeechError error) {
            if (error == null) {
                Log.i(TAG, "播放完成");
                if (callBackTTS!=null){
                    if (BasicActivity.is_Body){
                        callBackTTS.startTTS();
                    }else {
                        callBackTTS.stopTTS();
                    }
                    if (BasicActivity.Robot_Sport){
                        callBackTTS.startsmileface();
                    }else {
                        callBackTTS.startmianfg();
                    }
    ...
    public void setCallBackTTS(CallBackTTS callBackTTS) {
        this.callBackTTS = callBackTTS;
    }

    public static interface CallBackTTS{
        void stopTTS();
        void startTTS();
        void startsmileface();
        void startspeakface();
    ...
    public KqwSpeechCompound getKqwSpeechCompound(){
        if (kqwSpeechCompound==null){
            synchronized (KqwSpeechCompound.class){
                if (kqwSpeechCompound==null){
                    kqwSpeechCompound = new KqwSpeechCompound(mContext);
                }
            }
